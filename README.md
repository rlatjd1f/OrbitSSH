...
- 한국어·영어 UI 및 macOS 상단 메뉴 언어

언어는 **설정 → 일반 → 언어**에서 한국어 또는 English로 변경할 수 있습니다. 설정을 저장하면 사이드바, 연결 팝업, 터미널 안내, 업데이트 화면과 macOS 상단 메뉴가 선택한 언어로 함께 전환됩니다. 사용자가 등록한 장비 이름과 폴더 이름은 변경되지 않습니다.
